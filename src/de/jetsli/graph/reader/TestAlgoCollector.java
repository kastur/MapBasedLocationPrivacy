...
public class TestAlgoCollector {
    ...
    public TestAlgoCollector assertNull(RoutingAlgorithm algo, int from, int to) {
        ...
        if (p != null) {
            synchronized (this) {
                list.add(algo.getClass().getSimpleName() + " returns value where null is expected. " + "from:" + from + ", to:" + to);
            }
        }
        return this;
    }
    ...
    public TestAlgoCollector assertDistance(RoutingAlgorithm algo, int from, int to, double distance, int locations) {
        Path p = algo.clear().calcPath(from, to);
        if (p == null) {
            list.add(algo.getClass().getSimpleName() + " returns no path for " + "from:" + from + ", to:" + to);
            return this;
        } else if (Math.abs(p.distance() - distance) > 1e-2)
            list.add(algo.getClass().getSimpleName() + " returns path not matching the expected " + "distance of " + distance + "\t Returned was " + p.distance() + "\t (expected locations " + locations + ", was " + p.locations() + ") " + "from:" + from + ", to:" + to);
        ...
        if (Math.abs(p.locations() - locations) > 7)
            list.add(algo.getClass().getSimpleName() + " returns path not matching the expected " + "locations of " + locations + "\t Returned was " + p.locations() + "\t (expected distance " + distance + ", was " + p.distance() + ") " + "from:" + from + ", to:" + to);
        return this;
    }
